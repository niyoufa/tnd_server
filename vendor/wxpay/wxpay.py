...
class WxPayConf_pub(object):
    ...
    APPID = "wx1f5c2143c71eee36"
    #JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    APPSECRET = "0b681e45e5127dde6d78c1d3483ecb56"
    #受理商ID，身份标识
    MCHID = "1309964101"
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    KEY = "87B2447B04E5AA93F00C21A5B98F32A0"


    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = "https://www.renren.com/api/notify/wxpay"
    ...
    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    # SSLCERT_PATH = os.path.join(os.path.abspath('.'),'keys','apiclient_cert.pem')
    # SSLKEY_PATH = os.path.join(os.path.abspath('.'),'keys','apiclient_key.pem')
    SSLCERT_PATH = "/usr/share/nginx/renren/vendor/wxpay/keys/apiclient_cert.pem"
    SSLKEY_PATH = "/usr/share/nginx/renren/vendor/wxpay/keys/apiclient_key.pem"
...
class Wxinfo_pub(Common_util_pub):
    """获取微信信息类"""
    ...
    def create_access_token_url(self):
        return "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s"%(WxPayConf_pub.APPID,WxPayConf_pub.APPSECRET)

    def create_wx_info_url(self):
        return "https://api.weixin.qq.com/cgi-bin/user/info?access_token=%s&openid=%s"%(self.get_access_token(),self.openid)

    def get_access_token(self):
        """获取access_token"""
        url = self.create_access_token_url()
        data = HttpClient().get(url).decode()
        res = json.loads(data)
...
class Redpack_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        """生成接口参数xml"""
        if any(self.parameters[key] is None for key in ("mch_billno", "send_name", "re_openid", "total_amount", "total_num","wishing","client_ip",
                                                        "act_name","remark")):
            raise ValueError("missing parameter")
...
if __name__ == "__main__":
    ...
    info = Wxinfo_pub("oJav5wqle-P7E1-srHBTrL4seWB8")
    print(info.get_wx_info())